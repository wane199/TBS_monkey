...
#对参数设置尝试的代码
train$Rad <- cut(train$radscore, breaks = c(-Inf, 0.1834, Inf), labels = c("1","2"), right=TRUE, include.lowest=TRUE)
train$DurMonth <- cut(train$Durmon, breaks = c(-Inf, 108.00, Inf), labels = c("1","2"), right=TRUE, include.lowest=TRUE)
test$Rad <- cut(test$radscore, breaks = c(-Inf, 0.1834, Inf), labels = c("1","2"), right=TRUE, include.lowest=TRUE)
test$DurMonth <- cut(test$Durmon, breaks = c(-Inf, 108.00, Inf), labels = c("1","2"), right=TRUE, include.lowest=TRUE)
