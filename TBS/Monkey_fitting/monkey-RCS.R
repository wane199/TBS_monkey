...
p3 <- ggplot(dt, aes(Age, TBV)) + # , colour = Sex
  ...
  geom_smooth(
    ...
    method = "lm", formula = y ~ x + I((x - 5.6) * (x > 5.6))
  ) +
  scale_x_continuous(breaks = seq(0, 30, 1)) + # expand = c(0, 0),
  # scale_y_continuous(breaks = seq(45, 85, 5)) + # expand = c(0, 0),
  ...
  stat_poly_eq(
    aes(label = paste(after_stat(eq.label), after_stat(adj.rr.label), sep = "~~~~")),
    formula = y ~ x + I((x - 5.6) * (x > 5.6)), parse = TRUE
  ) +
  xlab("Age (year)") +
  ylab(bquote(Volume~(cm^3)))  + # Volume~(cm^3) Weight~(Kg)  TBV/Weight~(cm^3/kg) 'Uptake Value'~(kBq/cc) SUV~(g/ml) SUVr_refPons
  theme(
...
p4 <- ggplot(dt, aes(Age, TBV)) + # , colour = Sex
  ...
  geom_smooth(
    data = dt, mapping = aes(x = Age, y = TBV), # , color = Sex
    method = "lm", formula = y ~ x + I((x - 5.67) * (x > 5.67)) + I((x - 22.25) * (x > 22.25))
  ) +
  scale_x_continuous(breaks = seq(0, 30, 1)) + # expand = c(0, 0),
  # geom_vline(xintercept = 5.0, linetype = 2, color = "red") +
  ...
  stat_poly_eq(
    aes(label = paste(after_stat(eq.label), after_stat(adj.rr.label), sep = "~~~~")),
    formula = y ~ x + I((x - 5.67) * (x > 5.67)) + I((x - 22.25) * (x > 22.25)), parse = TRUE
  ) +
  xlab("Age (year)") +
  ylab(bquote(Volume~(cm^3)))  + # Volume~(cm^3) Weight~(Kg)  TBV/Weight~(cm^3/kg) 'Uptake Value'~(kBq/cc) SUV~(g/ml) 
  theme(
...
# 样条回归拟合效果
p7 <- ggplot(dt, aes(Age, TBV)) + # , colour = Sex
  geom_point() +
...
p51 <- ggplot(dt, aes(Age, TBV, colour = Sex)) +
  geom_point() +
  theme_classic() +
  ylab(bquote(TBV(cm^3))) + # TBV(cm^3) TBV/BW(cm^3/kg) Weight(kg) SUVr_whole_refPons Whole(cm^3/kg) SUVr_whole_refPons(KBq/cc)
  stat_smooth(method = lm, formula = y ~ rcs(x, 5)) +
...
p11 <- ggplot(dt, aes(Age, TBV)) + #  colour = Sex
  geom_point() +
