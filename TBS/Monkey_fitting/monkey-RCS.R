...
# 建立线性回归模型
model.lm <- lm(SUV_Whole ~ Age, data = dt) # 构建线性回归模型 SUVr_whole_refPons
summary(model.lm) # 查看回归模型结果，
p1 <- ggplot(dt, aes(Age, SUV_Whole)) + # , colour = Sex
  geom_point() +
  theme_classic() +
  ...
  xlab("Age (year)") +
  ylab(bquote(SUV~(g/ml)))  + # Volume~(cm^3) Weight~(Kg)  TBV/Weight~(cm^3/kg) 'Uptake Value'~(kBq/cc) SUV~(g/ml)
  theme(
...
p1
# 建立曲线方程(log,exp)
model.log <- lm(SUV_Whole ~ log(Age), data = dt) # 建立对数曲线方程
summary(model.log) # 查看模型概况
# 拟合曲线
p2 <- ggplot(dt, aes(Age, SUV_Whole)) + # , colour = Sex
  geom_point() +
  theme_classic() +
...
p2
model.log10 <- lm(SUV_Whole ~ log(Age), data = dt) # 建立指数曲线方程
summary(model.log10) # 查看模型概况
# 拟合曲线
ggplot(dt, aes(Age, SUV_Whole)) +
  geom_point() +
...
# 查看拟合效果
plot(dt$Age, dt$SUV_Whole, pch = 1, cex = 1.5)
abline(a = coef(model.lm)[1], b = coef(model.lm)[2], col = "red", lwd = 2.5)
plot(model.segmented, col = "blue", lwd = 2.5, add = T)

p3 <- ggplot(dt, aes(Age, SUV_Whole)) + # , colour = Sex
  geom_point() +
  geom_smooth(
    data = dt, mapping = aes(x = Age, y = SUV_Whole), # , color = Sex
    method = "lm", formula = y ~ x + I((x - 4.61) * (x > 4.61))
  ) +
  scale_x_continuous(breaks = seq(0, 30, 1)) + # expand = c(0, 0),
  # scale_y_continuous(breaks = seq(45, 85, 5)) + # expand = c(0, 0),
  ...
  stat_poly_eq(
    aes(label = paste(after_stat(eq.label), after_stat(adj.rr.label), sep = "~~~~")),
    formula = y ~ x + I((x - 4.61) * (x > 4.61)), parse = TRUE
  ) +
  xlab("Age (year)") +
...
p4 <- ggplot(dt, aes(Age, SUV_Whole)) + # , colour = Sex
  geom_point() +
  geom_smooth(
    data = dt, mapping = aes(x = Age, y = SUV_Whole), # , color = Sex
    method = "lm", formula = y ~ x + I((x - 5.38) * (x > 5.38)) + I((x - 21.5) * (x > 21.5))
  ) +
  scale_x_continuous(breaks = seq(0, 30, 1)) + # expand = c(0, 0),
  # geom_vline(xintercept = 5.0, linetype = 2, color = "red") +
  ...
  stat_poly_eq(
    aes(label = paste(after_stat(eq.label), after_stat(adj.rr.label), sep = "~~~~")),
    formula = y ~ x + I((x - 5.38) * (x > 5.38)) + I((x - 21.5) * (x > 21.5)), parse = TRUE
  ) +
  xlab("Age (year)") +
  ylab(bquote(SUV~(g/ml)))  + # Volume~(cm^3) Weight~(Kg)  TBV/Weight~(cm^3/kg) 'Uptake Value'~(kBq/cc) SUV~(g/ml)
  theme(
...
# 样条回归
model.spline <- lm(dt$SUV_Whole ~ rcs(dt$Age, 5)) # 建立样条回归，设置3~5个节点。+ factor(dt$Sex)
summary(model.spline) # 查看模型概况
# 样条回归拟合效果
p7 <- ggplot(dt, aes(Age, SUV_Whole)) + # , colour = Sex
  geom_point() +
...
# 计算拟合值
fml <- "SUV_Whole ~ rcs(Age, 5) + factor(Sex)"
...
dd <- datadist(dt)
options(datadist = "dd")
fit <- ols(SUV_Whole ~ rcs(Age, 5) + Sex, data = dt)
summary(fit)
an <- anova(fit)
...
library(dplyr)
ggplot(data = dt, aes(x = Age, y = SUV_Whole)) +
  geom_point() +
...
# filter(y == max(whole)) %>%
# # 在最高点添加标注
annotate("point", x = .$Age, y = .$SUV_Whole, shape = 16, size = 3) +
  annotate("text", x = .$Age, y = .$SUV_Whole, label = "Highest Point", vjust = -1.5)
# annotate("point", x = x_coordinate, y = y_coordinate, shape = 16, size = 3)

# triple in one
p51 <- ggplot(dt, aes(Age, SUV_Whole, colour = Sex)) +
  geom_point() +
  theme_classic() +
...
# Lowess函数建立局部加权回归
model.lowess <- lowess(dt$SUV_Whole ~ dt$Age) # 建立局部加权回归
summary(model.lowess) # 查看概况
# 查看拟合
p8 <- ggplot(dt, aes(Age, SUV_Whole)) + # , colour = Sex
  geom_point() +
...
model.gam <- gam(SUV_Whole ~ s(Age, k = 5, bs = "cs"), data = dt) # 建立gam模型 k = 4,k = 8,
summary(model.gam) # 查看模型概况
pr.gam <- predict(model.gam, dt) # 生成预测值
# 计算RSME和R方
data.frame(
  RMSE = RMSE(pr.gam, dt$SUV_Whole),
  R2 = R2(pr.gam, dt$SUV_Whole)
)
# 查看模型拟合情况
library(ggpmisc)
library(ggpubr)
...
p11 <- ggplot(dt, aes(Age, SUV_Whole)) + #  colour = Sex
  geom_point() +
