...
m0a <- lms(Frontal_Cortex, Age,
  data = dt,
  trans.x = TRUE, k = 3, families = "BCTo"
...
# 绘制平滑曲线，并添加数据分布热图
plot <- plotSimpleGamlss(Frontal_Cortex, Age, m1,
  data = dt,
  x.val = seq(0, 30, 2), # 定义数据分组
  xlim = c(0, 30), # 横轴的起止点为-2，23
  ylim = c(0, 0.15), val = 200
) # 纵轴的起止点为0，500

# 图像另存为
tiff(
  file = "Frontal_Cortex.tiff", res = 1400, width = 8800,
  height = 8800, compression = "lzw"
)
plot
dev.off()

mod <- gamlss(Frontal_Cortex ~ pb(Age),
  sigma.fo = ~ pb(Age),
  family = BCT, data = dt, method = mixed(1, 20)
)
plot(mod)
# rm(mod)
plot <- plotSimpleGamlss(Frontal_Cortex, Age, mod,
  data = dt,
  x.val = seq(0, 30, 2), # 定义数据分组
  xlim = c(0, 30), # 横轴的起止点为-2，23
  ylim = c(0.075, 0.115), val = 50
) # 纵轴的起止点为0，500
